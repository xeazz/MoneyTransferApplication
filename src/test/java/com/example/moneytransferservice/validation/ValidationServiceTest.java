...
public class ValidationServiceTest {
    ...
    public void senderCardIsNull() {
        TransferMoney transferMoney = new TransferMoney(null, "1", "1", "1", new Amount(100, "RUR"));
        assertThatThrownBy(() -> validationService.validateTransfer(transferMoney)).isInstanceOf(IncorrectInputDataException.class).hasMessage("Номер карты отправителя указан неверно!");
    }

    @Test
    @DisplayName("An empty expiration date should throw IncorrectInputDataException")
    public void validTillIsNull() {
        TransferMoney transferMoney = new TransferMoney("1", null, "1", "1", new Amount(100, "RUR"));
        assertThatThrownBy(() -> validationService.validateTransfer(transferMoney)).isInstanceOf(IncorrectInputDataException.class).hasMessage("Срок действия карты указан неверно!");
    }

    @Test
    @DisplayName("An empty CVV should throw IncorrectInputDataException")
    public void cardCvvIsNull() {
        TransferMoney transferMoney = new TransferMoney("1", "1", null, "1", new Amount(100, "RUR"));
        assertThatThrownBy(() -> validationService.validateTransfer(transferMoney)).isInstanceOf(IncorrectInputDataException.class).hasMessage("CVV указан неверно!");
    }

    @Test
    @DisplayName("An empty recipient card should throw IncorrectInputDataException")
    public void recipientCardIsNull() {
        TransferMoney transferMoney = new TransferMoney("1", "1", "1", null, new Amount(100, "RUR"));
        assertThatThrownBy(() -> validationService.validateTransfer(transferMoney)).isInstanceOf(IncorrectInputDataException.class).hasMessage("Номер карты получателя указан неверно!");
    }
    ...
    public void transferAmountValueIsIncorrect() {
        TransferMoney transferMoney = new TransferMoney("1", "1", "1", "1", new Amount(0, "RUR"));
        assertThatThrownBy(() -> validationService.validateTransfer(transferMoney)).isInstanceOf(IncorrectInputDataException.class).hasMessage("Сумма перевода указана некорректно!");
    }
    ...
    public void transferMoneyIsNull() {
        ...
        assertThatThrownBy(() -> validationService.validateTransfer(transferMoney)).isInstanceOf(InternalServerErrorException.class).hasMessage("Internal server error");
    }

    @Test
    @DisplayName("If amount transfer = null, then should throw InternalServerErrorException")
    public void amountIsNull() {
        TransferMoney transferMoney = new TransferMoney("1", "1", "1", "1", null);
        assertThatThrownBy(() -> validationService.validateTransfer(transferMoney)).isInstanceOf(InternalServerErrorException.class).hasMessage("Internal server error");
    }
    ...
    public void operationIdIsNull() {
        ...
        assertThatThrownBy(() -> validationService.validateConfirmOperation(transferOperation)).isInstanceOf(IncorrectInputDataException.class).hasMessage("Operation ID is null!");
    }
    ...
    public void transferOperationIsNull() {
        ...
        assertThatThrownBy(() -> validationService.validateConfirmOperation(transferOperation)).isInstanceOf(InternalServerErrorException.class).hasMessage("Error transfer, confirmation is null!");
    }
    ...
    public void invalidConfirmationCode() {
        ...
        assertThatThrownBy(() -> validationService.validateConfirmOperation(transferOperation)).isInstanceOf(InternalServerErrorException.class).hasMessage("Транзакция не была проведена!");
    }
}
